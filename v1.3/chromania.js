...
let outpanel, name, result, box, hex, settings, palette, inpanel, inp, sld;
...
window.onload = () => {
	...
	// palette
	palette = gid("palette");
	palette.addEventListener("click", add2palette);
...
function add2palette(e) {
	...
	if (typeof e === "string") {
		...
		setColor(e.target.dataset.hex);
		e.target.classList.add("remove-icon");
		if (palette.t)
			palette.t.classList.remove("remove-icon");
		palette.t = e.target;
		e.stopPropagation();
		return;
	} else
		color = hex[0].value.slice(0,7);
	
	if (palette[color]) return;
	
	let el = document.createElement("div");
	el.style.background = el.dataset.hex = color;
	el.innerHTML = '<svg width="24px" height="24px"><use href="#svg-remove"/></svg>';
	palette.insertBefore(el, palette.lastElementChild);
	palette[color] = true;
}
...
window.addEventListener("visibilitychange", () => {
	...
	if (document.visibilityState === "hidden") {
		...
		for (let x of palette.children)
			SVD.palette.push(x.dataset.hex);
		SVD.palette.pop(); // discard null value gotten from addbtn
...
function updateColors() {
	...
	palette.style.setProperty("--given", color);
...
function globalClick(e) {
	...
	// hide remove icon from palette item
	if (palette.t) {
		palette.t.classList.remove("remove-icon");
		palette.t = null;
	}
